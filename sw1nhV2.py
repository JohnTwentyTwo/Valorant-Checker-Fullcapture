...
def checker():
    ...
    create("Results")
    create(os.path.join("Results/", dt_string))
    create(os.path.join("Results/" + dt_string, "true,/"))
    create(os.path.join("Results/" + dt_string, "false,/"))
    folders = [
        "EU",
        "NA",
        "KR",
    ...
    for folder in folders:
        path = os.path.join("Results/" + dt_string + "/true,/",folder)
        create(path)
        path = os.path.join("Results/" + dt_string + "/true,/" + folder, "UserPass/")
        create(path)
    for folder in folders:
        path = os.path.join("Results/" + dt_string + "/false,/",folder)
        create(path)
        path = os.path.join("Results/" + dt_string + "/false,/" + folder, "UserPass/")
        create(path)
    os.system("cls")
    for line in lines:
        username = line.split(":")[0].replace('\n', '')
        password = line.split(":")[1].replace('\n', '')
        euwrite = open(f"Results//{dt_string}//checked.txt", "a+", encoding='utf-8')
    ...
    ╚═════╝░░░░╚═╝░░░╚═╝░░╚══════╝╚═╝░░╚══╝╚═╝░░╚═╝'''
        ...
        print(f"{Fore.RESET}          Current          :       ({Fore.LIGHTGREEN_EX}{username}:{password}{Fore.RESET})")
        ...
        print(f"{Fore.RESET}        ╚══════════════════════════════════════╝")        
        print(f"{Fore.RESET}  Note: -You can find result in true, folder") 
        print(f"{Fore.RESET}        -30 request/min")   
        print(f"{Fore.RESET}        -LATAM and BR region are included in NA region")  
        headers = OrderedDict({
        ...
        if "access_token" in r2.text:
            ...
            ratelimit += 1
            failed += 1
            rmtext = open(f"Results//{dt_string}//re_check.txt", "a+", encoding='utf-8')
...
#get Region + Accountlvl
        try:
            ...
            failed += 1
            failedtext = open(f"Results//{dt_string}//re_check.txt", "a+", encoding='utf-8')
            failedtext.write(f"{username}:{password};{GameName}#{Tag}\n")
            failedtext.close()
            print("        Error: cant get region")
            time.sleep(2)
            checked += 1
            continue     
...
#get last match
        try:
            ...
            failedtext.close()
            print("        Error: cant get last match")
            time.sleep(2)
            checked += 1
            continue
...
#output
        ...
        checked += 1
        good += 1
        euwrite = open(f"Results//{dt_string}//{EmailVerified}//{Region}//{category}.txt", "a+", encoding='utf-8')
        euwrite.write(f"{roait}")
        euwrite.close
        euwrite = open(f"results//{dt_string}//{EmailVerified}//{Region}//UserPass//{category}.txt", "a+", encoding='utf-8')
        euwrite.write(f"{username}:{password}\n")
        euwrite.close
checker()
